...
const createAdditionalPages = catchAsync(async (req, res) => {
  ...
  if (existingPage) {
    return res.status(403).json({ error: 'A page with this title already exists' });
  }

  const newPage = await AdditionalPage.create({ title, content, slug });
  res.status(201).json({ message: 'Additional Page Created Successfully', newPage });
});
...
const updateAdditionalPage = catchAsync(async (req, res) => {
  ...
  const page = await AdditionalPage.findByIdAndUpdate(id, update, { new: true });

  if (!page) {
    return res.status(404).json({ error: 'Page Not Found' });
  }

  res.status(200).json({ message: 'Additional Page Updated Successfully', page });
});
